...
#import "SemanticUnderstandingViewController.h"
...
@interface SpeechRecognitionViewController ()<IFlyRecognizerViewDelegate>
{
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.title = CustomLocalizedString(@"speechTitle", nil);
...
-(void)initIfly{
    //初始化语音识别控件
    _iflyRecognizerView = [[IFlyRecognizerView alloc] initWithCenter:CGPointMake(SCREEN_WIDTH/2, SCREEN_HEIGHT/2)];
...
- (IBAction)textUnderBtnHandler:(id)sender {
    SemanticUnderstandingViewController *vc = [[SemanticUnderstandingViewController alloc] init];
